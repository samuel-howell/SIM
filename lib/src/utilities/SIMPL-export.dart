...
class SIMPLExport {
  ...
  exportStoreCsv() async {
    ...
    if (kIsWeb) {
      ...
      Fluttertoast.showToast(
          msg:
              "Item CSV exported to ${generalDownloadDir.path}/SIMPL_S-EXPORT_$formattedDate.csv");
      print(storeList.toString());
  ...
  exportItemCsv() async {
    ...
    if (kIsWeb) {
      ...
      Fluttertoast.showToast(
          msg:
              "Item CSV exported to ${generalDownloadDir.path}/SIMPL_I-EXPORT_$formattedDate.csv");
...
//! THIS FILE IS HOW WE GET FIREBASE DATA WITHOUT HAVING TO BUILD A LISTVIEW OR ANY UI ELEMENT
  ...
  Future addSharedStoreDocs() async {
    QuerySnapshot querySnapshot = await db
        ...
        .get(); // only shows stores that have been sharedWith the currently signed in user.
    for (int i = 0; i < querySnapshot.docs.length; i++) {
      var doc = querySnapshot.docs[i];
      print(doc.get('name'));
